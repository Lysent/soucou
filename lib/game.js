...
const Game = function (canvas, state) {
    ...
    const always = () => state.alwaysRun.forEach(f => f(context));
    ...
    // utils
    const start = () => {
        // initialise always-run closures
        for (const [i, cl] of state.alwaysRun.entries()) {
            state.alwaysRun[i] = cl(context);
        }
    ...
    const addassets = obj => Object.keys(obj).forEach(k => state.assets[k] = obj[k]);
